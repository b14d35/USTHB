...
void ajout(FILE *f,FILE *s){
		...
		switch (t)
		{
		case 1: strcpy(traj.moy,"BUS");
		    break;
		case 2: strcpy(traj.moy,"TRAIN");
		    break;
        case 3: strcpy(traj.moy,"TGV");
            break;
		}
		printf("avec le prix de :\t");
		scanf("%d",&traj.prix);
		fprintf(f,"TRAJET:%d de distance %d par %s de  %d:%d:%d  a %d:%d:%d  de %s vers: %s de prix:%d  \n",traj.matricule,traj.distance,traj.moy,traj.heur_dpart.h,traj.heur_dpart.m,traj.heur_dpart.s,traj.heur_darriver.h,traj.heur_darriver.m,traj.heur_darriver.s,traj.wilactu,traj.wilaarr,traj.prix);
        printf("\n");
        fclose(f);
}
...
void sup(FILE *f, int x){
        ...
        do
        {
         fscanf(f,"TRAJET:%d de distance %d par %s de  %d:%d:%d  a %d:%d:%d  de %s vers: %s de prix:%d ",&traj.matricule,&traj.distance,&traj.moy,&traj.heur_dpart.h,&traj.heur_dpart.m,&traj.heur_dpart.s,&traj.heur_darriver.h,&traj.heur_darriver.m,&traj.heur_darriver.s,&traj.wilactu,&traj.wilaarr,&traj.prix);
         if(x!=traj.matricule){
			fprintf(fei,"TRAJET:%d de distance %d par %s de  %d:%d:%d  a %d:%d:%d  de %s vers: %s de prix:%d \n",traj.matricule,traj.distance,traj.moy,traj.heur_dpart.h,traj.heur_dpart.m,traj.heur_dpart.s,traj.heur_darriver.h,traj.heur_darriver.m,traj.heur_darriver.s,traj.wilactu,traj.wilaarr,traj.prix);
		      }
        } while (!feof(f));
		fclose(f);
		fclose(fei);
...
void affi(FILE *f){
    ...
    {
     fscanf(f,"TRAJET:%d de distance %d par %s de  %d:%d:%d  a %d:%d:%d  de %s vers: %s de prix:%d  ",&traj.matricule,&traj.distance,&traj.moy,&traj.heur_dpart.h,&traj.heur_dpart.m,&traj.heur_dpart.s,&traj.heur_darriver.h,&traj.heur_darriver.m,&traj.heur_darriver.s,&traj.wilactu,&traj.wilaarr,&traj.prix);
     printf("TRAJET:%d de distance %d par %s de  %d:%d:%d  a %d:%d:%d  de %s vers: %s de prix:%d  \n",traj.matricule,traj.distance,traj.moy,traj.heur_dpart.h,traj.heur_dpart.m,traj.heur_dpart.s,traj.heur_darriver.h,traj.heur_darriver.m,traj.heur_darriver.s,traj.wilactu,traj.wilaarr,traj.prix);
    } while (!feof(f));
    printf("\n");
    fclose(f);
}
...
int libre(FILE *g,int x)
{
    ...
    i=0;
      ...
      fclose(g);
      if (strcmpi(traj.moy,"BUS")==0 && i<56 || strcmpi(traj.moy,"TRAIN")==0 && i<201 || strcmpi(traj.moy,"TGV")==0 && i<151)
      {
        t=1;
      }
    printf("\n");
...
///Fonction qui permet de supprimer les reservations d'un trajet (si le trajet n'existe plus on doit supprimer les reservations)
...
int existe(FILE *f,int x)
{
    ...
    do
    {
		fscanf(f,"TRAJET:%d de distance %d par %s de  %d:%d:%d  a %d:%d:%d  de %s vers: %s de prix:%d  \n",&traj.matricule,&traj.distance,&traj.moy,&traj.heur_dpart.h,&traj.heur_dpart.m,&traj.heur_dpart.s,&traj.heur_darriver.h,&traj.heur_darriver.m,&traj.heur_darriver.s,&traj.wilactu,&traj.wilaarr,&traj.prix);
		if (x==traj.matricule)
		{
		   y=1;
		}
...
main()
{
    FILE *f,*g,*h,*s;
    int t,x,y;
///Fichier s me permet de generer et sauvegarder la position de mon code de trajet et la matricule des voyageurs
    s=fopen("inf.txt","a");
    rewind(s);
    if (feof(s))
    {
            ...
            do
            {
               ...
               scanf("%d",&t);
                ...
                case 2:
                        ...
                        else{printf("Ce trajet n'existe pas\n\n");}
                        if (exister(g,x)==1)
                            {
                                supc(g,h,x);
                            }
                        ...
                        ///S'il existe on le supprime sinon on affiche que le trajet n'existe pas
                        ///On verifie aussi s'il y a des reservations avec ce trajet on les annule toutes
                        system("pause");
                    break;
                case 3:
                        printf ("donnez le code de trajet :\t");
                        ...
                        system("pause");
                        ///On lit le code du trajet S'il existe on le modifie sinon on affiche qu'il n'existe pas
                    break;
                case 4: affi(f);
                        ///On fait un appel simple a la fonction qui affiche tout les trajets
                        system("pause");
                    break;
                case 5: goto g;
                    break;
        ...
        i:
            ...
            do
            {
              ...
              scanf("%d",&t);
                ...
                case 1: des(f,g);
                    system("pause");
                    ///On fait appel a la fonction qui affiche les trajets avec des places vides
                    break;
                case 2:
                    printf ("donnez le code de trajet :\t");
                    scanf("%d",&x);
                    ...
                    system("pause");
                    ///On cherche si le code du trajet existe puis on lui reserve
                    ///Puis on verifie s'il est disponible, ensuite on voit si il y a des places ou non
                    break;
                case 3:
			...
			if(existe(f,x)==1)
				{
				    ...
				    else{printf("Ce trajet est complet\n\n");}
				}
			else{printf("Ce trajet n'existe pas\n\n");}
				}
			else{printf("Cette matricule n'existe pas\n\n");}
			system("pause");
			///On cherche d'abord si le voyageur existe, puis il rentre le nouveau trajet pour le modifier
                    break;
                case 4:
			printf ("donnez votre matricule :\t");
			scanf("%d",&y);
			...
			system("pause");
			///On recherche si le passager existe a partir de sa matricule puis on le supprime
                    break;
                case 5: printf("\n");goto g;
                    break;
                case 6: return 0;
